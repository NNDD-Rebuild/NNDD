...
* ニコニコ市場機能を削除
* 検索APIのFQDN変更に対応
* このバージョンは正式なリリースとはしない
* 自動アップデートの対象としない

### 2023-12-03 v4.4.8 (Pre-release)
* コメント取得時のエラーを排除しとりあえず再生だけでも出来るようにした
* このバージョンは正式なリリースとはしない
* 自動アップデートの対象としない

### 2023-07-16 v4.4.7
* コメントの取得結果を強制的に成功として処理するように修正

### 2023-02-09 v4.4.6
* ニコニコ側でRSSのバグ修正が行われたため、v4.4.4で行った修正を戻した
...
* 【バグフィックス】ライブラリタブで検索中にフォルダを変更しても動画の一覧が更新されない不具合を修正(beta5)
...
* ダウンロードリスト
* -エコノミー時のダウンロード問い合わせを無視できるようにした（デフォルトはOFF）
* -動画のダウンロードに失敗した際のリトライを実装

* ライブラリ
* -ライブラリでの「更新」を、「コメントのみ」「ビデオ以外すべて」のどちらか選べるようにした
* -smおよびnm以外のビデオIDの動画も更新できるようにした（ビデオIDが数字だけのものは未対応）
* -ローカルのタグから動画を絞り込めるようにした

* Player
* -再生時の更新を、「コメントのみ」「コメント・タグ・説明文・ニコ割・市場」のどちらか選べるようにした
* -再生中に表示されるコメントの表示時間を調節できるようにした
* -Playerでコメントをクリックした時にコメントリストの該当するコメントを選択した状態にする機能を追加
* -コメントリストの項目をクリックするとコメントと同じテキストをコメント投稿のフィールドに設定するようにした

* その他
* -ログをファイルに出力するようにした

* 【バグフィックス】
* -DLが止まってしまう問題を修正（今度こそ！）
* -InfoViewで「Playerのサイズをビデオの大きさに合わせる」にチェックを入れた場合ですが .mp4動画は再生サイズが数ピクセルぐらいの極小になるのを修正
* -「再生のたびに動画情報を取得する」の項目にチェックを入れて、取得するとファイル名がおかしくなることがあるのを修正
* -投稿者コメントが250件以上表示されなかったのを修正
* -ライブラリのフォルダ名を変更するとフォルダ内のビデオが正しくDL済と判定されなかったのを修正

### 2009-02-21 NNDD v1.8.5
* 【バグフィックス】DLが100％で止まってしまう問題を修正（多分）

### 2009-02-17 NNDD v1.8.4
* ランキングの「合計」に対応
* 【バグフィックス】ニコニコ動画のランキングが取得できなくなっていたのを修正

### 2009-02-14 NNDD v1.8.3
* アフィきりかえを実装
* ランキングリストの右クリックメニューに「ストリーミング」を追加
* コメント表示件数の下限を10コメントに変更
* 【バグフィックス】DLリストでダウンロードが途中で止まってしまうのを修正（多分）

### 2009-02-10 NNDD v1.8.2
* コメントリストのスクロールを改善
* 【バグフィックス】ニコニコ動画のランキングが取得できなくなっていたのを修正
* 【バグフィックス】プレイリスト内の複数選択時の移動において、正しく移動できていない不具合を修正

### 2009-02-07 NNDD v1.8.1
* 【バグフィックス】ランキングのサムネイル画像が取得できなくなっていたのを修正
* 【バグフィックス】「動画投稿の新しい順/古い順」でタグ検索を行うと、「コメントの新しい/古い」で検索された結果がでてしまうのを修正
* 【バグフィックス】タグを検索すると結果が１ページしか表示されないのを修正
* 【バグフィックス】DLリストでカウントダウン中にDL済の項目を削除するとカウントダウンが止まるのを修正

### 2009-02-01 NNDD v1.8.0
* 市場の保存、表示を実装
* コメントの投稿を実装
* DLリストへのD&Dによる追加に対応
* DLリストへのクリップボードからのURL追加に対応
* PlayerへのD&Dの再生に対応
* ライブラリ内検索の複数ワードに対応
* DLリストに追加だけしてDLはしない設定が出来るようにした
* DLするコメント数を1000件に変更
* 表示するコメント数を設定できるように変更
* 【バグフィックス】特定の動画において、"DL済"で処理が止まってしまい、DLリストの次の項目に進まなくなってしまう不具合を修正
* 【バグフィックス】シークするとPlayerの大きさが変わってしまうのを修正
* 【バグフィックス】Player側で情報を更新するとコメントが２種類保存されてしまう不具合を修正
* 【バグフィックス】エコノミーキャンセル時のDLリストの挙動がおかしかったのを修正
* 【バグフィックス】再生中にPlayerを最小化するともとに戻らなくなる不具合を修正(Win)
* 【バグフィックス】再生中にPlayerを最小化できない不具合を修正(Mac)
...
* 【バグフィックス】Playerウィンドウの大きさを縦に変えたときにタブが操作できなくなるのを修正。

### 2008-11-25 NNDD ver1.5.2
* 【バグフィックス】停止させるタイプのニコ割で、ニコ割終了後に再生が再開しなかったのを修正。
* 【バグフィックス】Windowsにおいて、再生した直後のSWFを削除しようとすると「ファイルが開かれている」というエラーが出て削除できないのを修正。（flv、mp4は未対応）
* 【バグフィックス】ニコ割終了後に処理が重くなるのを修正。
* 【バグフィックス】ファイルの削除時に、リストが更新されない事があるのを修正。

### 2008-11-24 NNDD ver1.5.1
* 最新版にしても「最新版があります」というダイアログが出てしまうのを修正

### 2008-11-24 NNDD ver1.5.0
* SWFの一時停止、リピート、プレイリストへの追加に対応
* ユーザーニコ割
* ストア保存時の例外対処追加
* タグ、投稿者説明文対応

### 2008-11-16 NNDD ver1.4.5
* 音量をPlayerを閉じた後も保持するよう変更（swfは音量を調節できません。swf再生時は大きな音がなる事があるのでご注意ください。）

### 2008-11-15 NNDD ver1.4.4
* NGワードの非表示条件を「NGワードと完全一致」から「NGワードを含む」に変更
* ウィンドウ位置、サイズを保持するよう変更

### 2008-11-09 NNDD ver1.4.3
* 起動時のPlayer表示をしないように変更
* コメントリストを５０音順でソートした際に再生と同期しないよう変更
* メモリリーク対策（効果は今ひとつ）
* 【バグフィックス】検索で2ページ目以降が検索できていなかったのを修正
* 【バグフィックス】ランキング更新がキャンセルできないのを修正
* 【バグフィックス】SWF再生時に画像がパネルから少しずれる不具合を修正

### 2008-11-04 NNDD ver1.4.2
* 【バグフィックス】SWF再生時にコメントを隠す・表示をするとSWFがパネルの位置に追従しないのを修正
* 【バグフィックス】保存先フォルダが無い状態でのv1.4系初回起動のライブラリ作成で先に進めなかったのを修正
* 【バグフィックス】プレイリスト再生中に１曲リピートするとプレイリストの内容が消えていたのを修正

### 2008-11-03 NNDD ver1.4.1
* 【バグフィックス】Windowsでフォルダの追加等の操作が出来なかったのを修正

### 2008-11-03 NNDD ver1.4
* プレイリスト順序変更に対応
* ダウンロードタブとダウンロード済みアイテムの同期するように変更
* ダウンロード済みリストに作成日追加
* ダウンロード済みファイル管理機能強化（フォルダ追加・削除・移動・名前変更）
* 動画とコメントリストの同期に対応
* ダウンロードタブの「再生」ボタン新設
* ログを設定タブに統合
* アップデート通知ダイアログを変更
* 【バグフィックス】SWF再生時にコメントが流れないのを修正
* 【バグフィックス】ライブラリタブで最大化時に右のリストが左側のリストに隠れてしまっていたのを修正
* 【バグフィックス】プレイリストの名前を変えると古い名前のプレイリストが残ってしまっていたのを修正

### 2008-10-27 NNDD ver1.3.1
* 【バグフィックス】Playerウィンドウのチェックボックスを記憶できていなかったのを修正
* 【バグフィックス】Windowsでログインウィンドウが一番手前に来ていなかったのを修正
* 【バグフィックス】検索ができていなかったのを修正

### 2008-10-26 NNDD ver1.3
* プレイリスト実装（.m3uファイル、FullPath限定）
* 項目の削除（右クリックメニュー、DelキーおよびBSキー）
* ダウンロードタブでのDL済み項目再生（右クリック）
* チェックボックス状態記憶
* Player右側タブの一発隠し（動画を再生している部分をダブルクリックすると"隠す"or"表示"を切り替えられます）
* 起動時にログインウィンドウが一番手前に来るように変更
* 【バグフィックス】Macでタイトルに"?"を含む場合に保存できていなかったのを修正（"?"を"-"に置換）
* 【バグフィックス】検索単語をURLエンコードしていなかったのを修正
* 【バグフィックス】アップデート確認用URLが間違っていたのを修正

### 2008-10-19 NNDD ver1.2
* 「検索」にタグ検索を新設・次のページ、前のページを見れるようにした。
* 【おまけ】再生ウィンドウのフルスクリーン化

### 2008-10-13 NNDD ver1.1.1
* NGリストを実装
* コメントの表示を調整
* ランキング・検索結果表示時に再生数・コメント数・マイリスト数・最近のコメントを表示
* 【バグフィックス】（秋）変更後、ランキングのサムネイルが取得できなくなっていたのを修正
* 【バグフィックス(v1.1.1)】コメント表示時に「ue」コマンドを正しく検出していなかったのを修正

### 2008-10-05 NNDD ver1.0 正式版
* アイコンを設定（[http://twitter.com/keiith:title=@keiith]嬢にデザインしていただきました！）
* 起動時のバージョンチェックを実装
* タブ切り替え後もDL済リストのツリーの状態を保持するよう変更
* 【バグフィックス】SWF再生時に音だけが流れて再生されない問題を修正（SWF再生時にウィンドウサイズを自動的にリサイズするようにした）
* 【バグフィックス】一部環境において、検索時にサムネイルが取得できずエラーになっていたのを修正
* 【バグフィックス】エラー時に表示される文言を修正

### 2008-10-04 NNDD ver1.0 RC
* ニコニコ動画内の動画検索を実装。
* コメントを新たにダウンロードする際、コメントファイルの名前をローカルのビデオの名前に合わせるように変更。
* 【バグフィックス】投稿者コメントを、v1.0系で[owner]として見ており、v1.0以前の[Owner]が読み込めていなかったので、v1.0でも[Owner]を使うように修正。（結果、v1.0 beta ~ beta4でダウンロードした投稿者コメントが読み込めなくなります。お手数ですが"o"を"O"に変えるか、コメントをダウンロードし直してください。）
* 【バグフィックス】投稿者コメントの保存時に"."を余分につけていたのを修正。
* 【バグフィックス】「フォルダ内の項目をすべて表示」のチェックを入れたとき、ビデオのリストにフォルダが表示されていたのを修正。
* 【仕様変更対応】変更点の二つ目の実装で、「ニコニコ動画（夏）の時に取得した動画のコメントがニコニコ動画（秋）というファイル名になっていて新しい方のコメントがロードされない」のを解決しました。

### 2008-09-27 NNDD ver1.0 beta4
* SWFを再生する際、ウィンドウを大きくするとSWFが画面の左端に寄っていたのを中央に表示するよう変更
* 【バグフィックス】一時停止中にシークすると、コメントだけが流れてしまう不具合を修正
* 【バグフィックス】「フォルダ内の項目をすべて表示」にチェックを入れた際にランキングリストの更新を行っていたのを、ダウンロード済みリストを更新するように修正

### 2008-09-22 NNDD ver1.0 beta3
* コメントの影にドロップシャドウをつけるように変更
* ランキングリストのダブルクリックでダウンロードできるよう変更
* 【バグフィックス】一時停止するとコメント用の表示タイミングがずれるのを修正
* 【バグフィックス】SWFを「ダウンロードしながら再生」で再生した後、再生ボタンを押すとエラーが発生していたのを修正

### 2008-09-21 NNDD ver1.0 beta2
NNDD2からNNDDに名称を変更します。
NNDD2からの変更点
* ニコニコ動画のランキングに表示される「カテゴリ」に対応
* コメントの大きさ・色・場所に対応
* SWFの再生に対応
* TOP100ダウンロードの削除

### 2008-08-11 ver0.91
* 投稿者コメントの取得・表示に対応。コメントタブには、"[This is Owner Comment]"というタグが付加された状態で登録されます。動画に出力される際はタグは付加されません。
* 【バグフィックス】ムービーが何も選択されていない状態で再生ボタンを押すとエラーを出力していたのを、エラーを出力しないように修正。
* 【バグフィックス】ログインに失敗すると「ログイン」ボタンがクリックできなくなり、その先に進めなくなる不具合を修正。

### 2008-08-08 ver0.901
* 【バグフィックス】インクリメンタルサーチ中にムービーを再生しようとすると、ダブルクリックしたムービーとは違うムービーが再生されてしまう不具合を修正

### 2008-08-07 ver0.9
* GUI変更（ランキングリストをタブに統合）
* ニコニコ動画ランキング取得時の対象変更対応
* ダウンロード中のムービー再生
* ニコチャートダウンロード時の対象変更対応
* DL済みリストのインクリメンタルサーチ
* 【バグフィックス】常に最前面のチェックを外したときの挙動がおかしいのを修正
* 【バグフィックス】コメントのみの取得がおかしかったのを修正

### 2008-07-27 ver0.83
* ダウンロードしたビデオの保存時にビデオIDを付加するよう変更しました。（例えば、sm12345というFLVをダウンロードした場合、「ビデオのタイトル - [sm672747].flv」というファイル名でFLVが保存されます。）
* ファイル名に付加したビデオIDをつかって、ダウンロード済みのビデオに対応する最新のコメントを取得できるようにしました。（ダウンロードする際、既存のコメントファイルが上書きされます。必要な場合はバックアップをとってください。）
* ビデオ再生時に表示されるコメントを白抜き文字に変更しました。

### 2008-07-22 ver0.82.1
* ソースリストの表示がおかしくなるのを修正
* リピートできていなかったのを修正

### 2008-07-21 ver0.82
* 再生用ウィンドウをメインウィンドウから分離
* コメントに改行が含まれていればそれを反映するように変更（コメントの表示にLabelを使っていたのをTextに変更）
* コメント出力の速度等微調整
...
### 2008-05 ころ
* AdobeAIR(FlexBuilder)でニコニコ動画から動画をダウンロードしてやる！とか思い始める。